...
from examples.co_delfos_productos_srf_transformar_remittances_order_status.schemas.dynamodb_remittances_order_status_schema import schema_full
    ...
    data = [
        Row(
            Item=Row(
                ...
                customer_nombre=Row(S="John Doe"),
                order_date=Row(S="2024-01-15T10:30:00Z"),
                state_date=Row(S="2024-01-15T11:00:00Z")
            )
        ),
        Row(
            Item=Row(
                order_id=Row(S="ORDER002"),
                remittance_id=Row(S="REM002"),
                customer_nombre=Row(S="Jane Smith"),
                order_date=Row(S="2024-01-15T12:00:00Z"),
                state_date=Row(S="2024-01-15T12:30:00Z")
            )
        )
    ]
...
def test_full_etl_pipeline(glue_context, mock_dynamodb_full_data, constants, job_params):
    """Test complete ETL pipeline for FULL process type."""
    ...
    assert job_config.process_date == "2024-01-15"
...
def test_full_etl_pipeline_empty_data(spark, glue_context, constants, job_params):
    """Test ETL pipeline handles empty data gracefully for FULL process."""
